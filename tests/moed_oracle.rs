...
fn moed_of_matches_hebcal_2026() {
    ...
    for (date, want) in cases {
        assert_eq!(moed_of(*date, d), *want, "moed_of({:?}) mismatch", date);
    }
}
...
fn eretz_yisrael_has_no_second_days() {
    ...
    assert_eq!(
        moed_of(hd(5787, 7, 23), ey),
        None,
        "no Simchas Torah day in EY"
    );
...
fn three_weeks_and_nine_days_ranges() {
    ...
    assert!(
        in_three_weeks(hd(5786, 4, 17)),
        "17 Tammuz starts the Three Weeks"
    );
    assert!(
        in_three_weeks(hd(5786, 5, 1)),
        "1 Av is inside the Three Weeks"
    );
    assert!(in_three_weeks(hd(5786, 5, 9)), "9 Av is the last day");
    assert!(!in_three_weeks(hd(5786, 5, 10)), "10 Av is after");

    assert!(
        !in_nine_days(hd(5786, 4, 29)),
        "29 Tammuz (last of Tammuz) is before the Nine Days"
    );
